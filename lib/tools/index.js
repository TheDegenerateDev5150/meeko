 ...
 * @returns  {Promise}
 */
/* istanbul ignore next -- 默认函数作为值传入 Promise.race，永远不会被调用 */
const noopAsync = async function () {}
tools.race = async function (fn = noopAsync, timeout = 5000, errorObj = [500, 'timeout']) {
  ...
  const typeCheck = function (i, valA, valB, addToC) {
    ...
    switch ((valB.type || 'string').toLow()) {
      ...
      case 'string':
        ...
        /* istanbul ignore next -- 经过 typeof 与 ==='' 判断后，valA 必为非空字符串，|| 右侧不可达 */
    ...
    for (i = 0; i < size; i++) {
      ...
      if (buffer === '') {
        ...
        if (code <= 255) {
          ...
          chain = dict[code]
          /* istanbul ignore next -- dict 为数组，dict[code] 只会是字符串或 undefined，不会为 null */
          if (chain === null) {
...
const cFn = function cFn (s, fc, dimNum, bc, isUnderline) {
  ...
  /* istanbul ignore next -- cFn 为私有函数，所有调用方均传入 truthy 的 fc，': '\x1b[37m'' 侧不可达 */
